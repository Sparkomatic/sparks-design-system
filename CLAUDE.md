...
```bash
source ~/.nvm/nvm.sh && npm run storybook
# Opens at http://localhost:6006
```

**Run the log server (captures Storybook browser runtime errors to storybook.log):**
```bash
source ~/.nvm/nvm.sh && npm run logs
# Run in a second terminal alongside npm run storybook
```

Two log files are written (both gitignored):
- `storybook-server.log` — Vite/build/module-resolution errors (server side, written by npm run storybook via tee)
- `storybook.log` — React render errors, console.error, JS exceptions (browser side, written by log-server)

## Project structure

```
sparks-design-system/
  package.json                  ← npm workspaces root; root deps include Radix + Storybook
  tsconfig.json                 ← root TS config for Storybook/Vite (browser target)
  log-server.js                 ← dev log server (port 7777) — run with npm run logs
  storybook-server.log          ← Vite/build errors from npm run storybook (gitignored)
  storybook.log                 ← browser runtime errors from log-server (gitignored)
  .storybook/
    main.ts                     ← Storybook config; stories glob, addons, Vite framework
    preview.ts                  ← global token CSS imports + sendLog() error capture
  registry/
    ...
    components/
      button/
        ...
        button.stories.tsx      ← Storybook stories (stays in repo, not copied to consumer)
  packages/
    cli/
...
3. Create `registry/components/<name>/<name>.stories.tsx` — Storybook stories (not copied to consumers)
4. Add entry to `registry/registry.json` — include `files`, `dependencies`, `tokens`
5. If the component has npm dependencies, add them to the **root** `package.json` (not just the CLI workspace) so Vite/Storybook can resolve them: `npm install <pkg>`
6. Test: run `sparks-ds add <name>` in a fresh temp directory
7. Update README.md component table
...
### Figma MCP (per-session, live)

The `figma-remote-mcp` MCP server is configured and available as a complement to the manifest. Use it during interactive sessions to inspect the current state of a specific component directly.

It does **not** detect what's selected in Figma — you must supply a node URL. Workflow:
1. Right-click a node in Figma → **Copy link**
2. Paste the URL (it includes `?node-id=XX-XX`) into the chat
3. Claude calls `get_design_context` with the extracted `fileKey` and `nodeId`

Figma file key: `OzEZwWy8CQFot6GcTVeg8f`

The manifest remains the primary source of truth (it's always available, versioned, and offline). MCP is useful for ad-hoc visual inspection during component authoring.

### Sync workflow

The manifest is generated by `scripts/sync-figma.ts`.

**Without variables (components + styles only):**
```bash
source ~/.nvm/nvm.sh && npx tsx scripts/sync-figma.ts
```

**With variables (recommended — free, no paid plan needed):**
1. Open the `claude-figma-variables-and-styles-plugin` in Figma
2. Click **Export** — this downloads a JSON file of all local variables
3. Pass that file to the sync script:
```bash
source ~/.nvm/nvm.sh && npx tsx scripts/sync-figma.ts --variables ~/Downloads/variables-export.json
```

The `--variables` flag reads variables from the plugin export (which uses `figma.variables.getLocalVariables()` — free on any plan) rather than the REST API (which requires a paid plan). When the flag is provided the REST API variables endpoint is skipped entirely.

Commit the result so the snapshot is always available without Figma being open.
...
The `file_variables:read` scope (Variables REST API) requires a paid Figma plan and is no longer needed — use `--variables` with the plugin export instead.
